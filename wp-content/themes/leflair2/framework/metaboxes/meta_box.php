 ...
 */
 ...
 // ajax function 
add_action( 'wp_ajax_custom_tax_select', 'custom_tax_select' );
add_action( 'wp_ajax_nopriv_custom_tax_select', 'custom_tax_select' );
function custom_tax_select(){
	$data = $_POST;
	if($data['current_trigger'] > 0) {
	$taxonomies = array( 
		$data['tax'],
	);
	$args = array(					
	...
	if(!empty($terms)) foreach($terms as $t) {
		$selected = ($t->term_id == $data['current']) ? 'selected="selected"' : '';
		$out .= sprintf('<option value="%s" %s >%s</option>',$t->term_id,$selected,$t->name);
	}	
	//var_dump($terms);
	}else {
		$out = '<option>Select Area</option>';
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	$document_id = isset( $field['document_id'] ) ? $field['document_id'] : null;
	$post_type = isset( $field['post_type'] ) ? $field['post_type'] : null;
	$tax_type = isset( $field['tax_type'] ) ? $field['tax_type'] : null;
	$trigger_id = isset( $field['trigger_id'] ) ? $field['trigger_id'] : null;
	$options = isset( $field['options'] ) ? $field['options'] : null;

    $is_child = isset( $field['is_child'] ) ? $field['is_child'] : null;
	//Custom VinhDQ
	if($document_id > 0)  $options  = get_document_item($document_id);
	...
	switch( $type ) {
		...
		case 'image_serialize':
			 ...
			 if($images)
			 {
				 ...
				 {
					echo '<div  style="max-width:200px;"><a target="_blank" href="'.$img.'"><img style="max-width:100%;" src="'.$img.'" /></a></div>';
				 }
			 }
             echo '<input type="hidden" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . esc_attr( $meta ) . '" class="regular-text" size="30" />';
		break;
		case 'url':
		...
		case 'post_chosen_one':
			//var_dump($meta);
			echo '<select data-placeholder="Select One" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"' , $type == 'post_chosen_one' ? ' class="chosen"' : '' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , '>
					<option value=""></option>'; // Select One
					
			$posts = get_posts( array( 'post_type' => $post_type, 'posts_per_page' => -1, 'orderby' => 'name', 'order' => 'ASC' ) );
			foreach ( $posts as $item )
				echo '<option value="' . $item->ID . '"' . selected( $meta, $item->ID, false ) . '>' . $item->post_title .  '</option>';
			$post_type_object = get_post_type_object( $post_type );
			echo '</select> &nbsp;<span class="description"><a href="' . admin_url( 'edit.php?post_type=' . $post_type . '">Manage ' . $post_type_object->label ) . '</a></span><br />' . $desc;
		break;
		...
		case 'tax_select':
			...
			echo '</select> &nbsp;<span class="description"><a href="'.get_bloginfo( 'url' ) . '/wp-admin/edit-tags.php?taxonomy=' . $id . '">Manage ' . $taxonomy->label . '</a></span>
				<br />' . $desc;
		break;
		case 'tax_select_type1':
			echo '<select  data-target="ajax_tax" data-tax="'.$tax_type.'"  class="ajax_tax_parent" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '">
					<option value="">Select One</option>'; // Select One
			$terms = get_terms( $tax_type, 'get=all' );
			$current = get_post_meta( get_the_ID(), $id ,true);
			$taxonomy = get_taxonomy( $tax_type );
			foreach ( $terms as $term ) {
				if($term->parent == 0) {
					$term_value = $taxonomy->hierarchical ? $term->term_id : $term->slug;
					echo '<option value="' . $term_value . '"' . selected( $current, $term_value, false ) . '>' . $term->name . '</option>'; 
				}
			}
			echo '</select> &nbsp;<span class="description"><a href="'.get_bloginfo( 'url' ) . '/wp-admin/edit-tags.php?taxonomy=' . $tax_type . '">Manage ' . $taxonomy->label . '</a></span>
				<br />' . $desc;
		break;
		
		case 'tax_select_type2':
			echo '<select  name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '">
					<option value="">Select One</option>'; // Select One
			$terms = get_terms( $tax_type, 'get=all' );
			$current = get_post_meta( get_the_ID(), $id ,true);
			...
			foreach ( $terms as $term ) {
				if($term->parent == 0) {
					//$term_value = $taxonomy->hierarchical ? $term->term_id : $term->slug;
					echo '<option value="' . $term->term_id . '"' . selected( $current, $term->term_id, false ) . '>' . $term->name . '</option>'; 
				}
			}
			echo '</select> &nbsp;<span class="description"><a href="'.get_bloginfo( 'url' ) . '/wp-admin/edit-tags.php?taxonomy=' . $tax_type . '">Manage ' . $taxonomy->label . '</a></span>
				<br />' . $desc;
		...
		case 'tax_select_type_ajax':
			$current = get_post_meta( get_the_ID(), $id ,true);
			echo '<select data-tax="'.$tax_type.'" data-trigger="'.$trigger_id.'" data-current="'.$current.'" class="ajax_tax" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '">
					<option value="">Loading item</option>'; // Select One
			echo '</select> 
				<br />' . $desc;
		break;
		...
		// repeatable
		case 'document':
			build_document_field($id,$name,$meta,$type,$options,$document_id,$is_child);
		break;
		case 'variation_product':
			build_variation_product_field($id,$name,$meta,$type,$options,$document_id,$is_child);
		break;
		
		case 'res_products':
			build_res_products($id,$name,$meta,$type,$options,$document_id,$is_child);
		break;
...
function build_res_products($id,$name,$meta,$type,$options,$document_id,$is_child,$desc=""){ 
	global $post;
	$post_id = $post->ID;
	$sysbol = get_woocommerce_currency_symbol();
	...
	if ( $the_query->have_posts() ) {
		while ( $the_query->have_posts() ) {
			$the_query->the_post(); 
			//$feature_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' ); 
			//echo get_the_title();
			$price = get_post_meta(get_the_ID(),'_price',true);
			$link = get_edit_post_link(get_the_ID());
			$output .= sprintf('<tr> <td>%s</td> <td><a target="_blank" href="%s">%s</a></td> <td>%s</td> </tr>',get_the_ID(),$link,get_the_title(),$sysbol.$price);
			}
	}else {
		$output .= '<tr> <td colspan="3">No product founds.</td></tr>';
	}
	$output .= "</tbody>";
	$output .= "</table>";
	echo $output;
...
function build_variation_product_field($id,$name,$meta,$type,$options,$document_id,$is_child,$desc=""){
	
	$multiple = false;
    $options = array();
	if($type == 'variation_product') {
		$options = get_woo_product();
	}
    
	echo '<select name="' . esc_attr( $name ) . '"  placeholder="Your Favorite Type of Bear" id="' . esc_attr( $id ) . '"' , $type == 'variation_product' ? ' class="chosen-select-deselect"' : '' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , '>
...
function get_woo_product($atts = array() , $content = null)
{   
	$product_ar = array();
	$post_id = $_REQUEST['post'];
    $args = array( 'post_type' => 'product','post_status'=>'publish', 'posts_per_page' => 10, 'orderby' => 'name','post__not_in' => array($post_id) );
    $loop = new WP_Query( $args );
    while ( $loop->have_posts() ) : $loop->the_post(); 
    //global $post; 
	$sku = get_post_meta($loop->post->ID,'_sku',true);
	$item['value'] = $loop->post->ID;
	$item['label'] = (($sku != "") ? $sku."-" : "" ). $loop->post->post_title;
	$product_ar[] = $item;
    endwhile; 
    wp_reset_query(); 
	return $product_ar;
...
function build_document_field($id,$name,$meta,$type,$options,$document_id,$is_child,$desc=""){

	$multiple = true;
    $meta = ($meta != "") ? json_decode($meta,1) : array();
    //var_dump($meta);
    $name = $name."[]";
    //$id = $id."[]";
    if(empty($options)) { echo '<p>This section not have any files</p>'; } else {
    $cls = ($is_child == 1) ? 'child-option' : '';
    echo '<div class="'.$cls.'">';
	echo '<select name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"' , $type == 'document' ? ' class="chosen"' : '' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , '>
	       '; // Select One  <option value="">Select One</option>
			foreach ( $options as $option ) {
                $selected = '';
                if(in_array($option['value'],$meta)) {
                    echo '<option selected="selected" value="' . $option['value'] . '">' . $option['label'] . '</option>';
                }else {
                    echo '<option value="' . $option['value'] . '">' . $option['label'] . '</option>';
                }
...
if ( $the_query->have_posts() ) {
	while ( $the_query->have_posts() ) {
		$the_query->the_post();
		$option[] = array('value' => get_the_ID(),'label' => get_the_title());
	}
}
wp_reset_postdata();
return $option;
...
class Custom_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			if ( in_array( true, array(
				...
				meta_box_find_field_type( 'post_chosen_one', $this->fields ),
				meta_box_find_field_type( 'document', $this->fields ),
			) ) ) {
			...
			wp_enqueue_script( 'meta_box', CUSTOM_METABOXES_DIR . '/js/metabox.js', $deps );
			$translation_array = array( 'ajax_url' => admin_url('admin-ajax.php') , 'site_url' => site_url() );
			wp_localize_script( 'meta_box', 'jsdata', $translation_array );
		}
	}
	...
	function meta_box_callback() {
		...
		foreach ( $this->fields as $field) {
            if ( $field['type'] == 'section_wpdm' ) {
                echo '<tr>
						<td colspan="2">
							<h4 class="meta_sub_header">' . $field['label'] . '</h4>
						</td>
					</tr>';
            }
                ...
                $style = ($field['is_child'] == 1) ? 'style="padding-left:25px;"':'';
                echo '<tr>
						<th style="width:20%"><label  '.$style.'  for="' . $field['id'] . '">' . $field['label'] . '</label></th>
						<td>';
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				if ( isset( $new ) && '' == $new && $old ) {
                    ...
                    if( in_array( $field['type'], array( 'document' ) ) ) {
                        $new = json_encode($_POST[$field['id']]);
                    }
					update_post_meta( $post_id, $field['id'], $new );
				}
			}
